...
void wait_for_twi() {
  ...
  while ((*myTWCR & 0x80) == 0x00) {
    timeout++;
    if (timeout > 50000) {
      break; // Escape the infinite loop if motor noise crashes the I2C!
    }
  }
}
void i2c_start() {
...
// The RTC requires time in Binary-Coded Decimal (BCD). This converts normal numbers.
unsigned char decimal_to_bcd(int dec) {
  return ((dec / 10) << 4) + (dec % 10);
}

// Function to force a new time into the RTC memory
void set_rtc_time(int hr, int min, int sec) {
  i2c_start();
  i2c_write(0xD0); // 0xD0 is the DS1307 write address
  i2c_write(0x00); // Point to Register 0 (Seconds)
  
  // Shove the new time into the registers in order
  i2c_write(decimal_to_bcd(sec)); 
  i2c_write(decimal_to_bcd(min));
  i2c_write(decimal_to_bcd(hr)); // Use 24-hour format!
...
void debounce() { for(volatile long int i = 0; i < 150000; i++); }
int readStartButton() {
...
// --- HARDWARE INTERRUPT FOR START BUTTON (PIN 18) ---
void startButtonISR() {
  // Simple software debounce
  for(volatile long int i = 0; i < 150000; i++); 
...
void setup() {
  ...
  set_rtc_time(16, 50, 0);
  init_LEDs();
  init_buttons();
  attachInterrupt(digitalPinToInterrupt(18), startButtonISR, RISING);
  init_motor();
  setup_water_sensor();
  init_LCD();
  timer1_init();
...
// Counter to track 60 seconds (30 ticks of our 2-second timer)
int minute_counter = 30; 

void loop() {
  // 1. READ RESET BUTTON (Start button is now handled by ISR!)
  ...
  if (timer_heartbeat == 1) {
      if (current_state != OFF) {
        ...
        minute_counter++; // Increase the clock
      }
      timer_heartbeat = 0; 
  }
  ...
  // 4. LCD & SERIAL OUTPUT (Updates exactly once every 60 seconds)
  // FOR THE VIDEO DEMO: Change "minute_counter >= 30" to "minute_counter >= 1" so it updates instantly!
  if (current_state != OFF && minute_counter >= 30) {
      get_rtc_time();
      ...
      minute_counter = 0; // Reset the 60-second clock
  }
}